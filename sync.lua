...
function Sync.queueAction(book_id, action_type, data)
    ...
    -- Deduplicate: replace existing action of same type for same book
    for i, item in ipairs(queue) do
        if item.book_id == book_id and item.action_type == action_type then
            queue[i].data = data
            saveQueue(queue)
            logger.info("StoryGraph: Updated queued " .. action_type .. " for book " .. book_id)
            return
        end
    end
...
function Sync.shouldSkip(book_id, action_type, new_data)
    ...
    if action_type == "status" and last_sync.status == new_data.status then
        return true
    end

    if action_type == "progress" and last_sync.percentage == new_data.percentage then
        return true
    end
...
-- Returns true on full success, or false plus an error message on failure
