...
describe("Main plugin helpers", function()
    ...
    describe("getReadingPercentage", function()
        it("returns correct percentage for midpoint", function()
            local pct = getReadingPercentage(150, 300)
            assert.are.equal(50, pct)
        end)

        it("returns 0 for first page", function()
            local pct = getReadingPercentage(1, 300)
            assert.are.equal(0, pct)
        end)

        it("returns 100 for last page", function()
            local pct = getReadingPercentage(300, 300)
            assert.are.equal(100, pct)
        end)

        it("returns 0 when total_pages is 0", function()
            local pct = getReadingPercentage(50, 0)
            assert.are.equal(0, pct)
        end)

        it("returns 0 when total_pages is nil", function()
            local pct = getReadingPercentage(50, nil)
            assert.are.equal(0, pct)
        end)

        it("floors fractional percentages", function()
            local pct = getReadingPercentage(1, 3)
            assert.are.equal(33, pct)
        end)
    end)
end)
